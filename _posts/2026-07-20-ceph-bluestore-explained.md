...
category: storage
tags: [ceph, bluestore, storage, distributed storage, rocksdb, smr, zoned storage, ublk, vstart, cephadm, debugging]
---
...
> - Aghayev, Weil, Ganger, Amvrosiadis, ["Reconciling LSM-Trees with Modern Hard Drives using BlueFS"](https://www.pdl.cmu.edu/PDL-FTP/FS/CMU-PDL-19-102.pdf), CMU-PDL-19-102, Apr 2019 — BlueFS running RocksDB on SMR drives; the study behind the zoned-mode work in Part 4.
...
filesystem in between — with metadata in an embedded RocksDB. Five
views, in order: where BlueStore sits and how to deploy it (Part 1);
the four environments a change passes through, and how to debug them
(Part 2); components, data structures and the read / write /
transaction paths (Part 3); a design exploration of SMR / zoned disk
support, ublk versus native (Part 4); and testing BlueStore in
isolation, no cluster (Part 5). Assumes storage and kernel background,
no Ceph.
...
# Part 4: supporting SMR / zoned disks — a design exploration
...
# Part 5: hands-on — testing and inspecting BlueStore in isolation
...
run it against a scratch directory or a disk.
...
The full set is in *Observing BlueFS* (Part 3); these are the offline
ones worth knowing:
...
Debug logs narrate the exact code paths from Part 3; set them via env
