...
RocksDB. This post explains BlueStore five ways: from the outside (what
Ceph is for, where BlueStore sits, how to deploy and verify it on a Fedora
VM), from a developer's chair (how to build, run, debug and install Ceph —
the four environments a change passes through), from the inside (its
components, data structures, and the read / write / transaction paths
through the code), forward-looking (a design exploration of supporting
SMR / zoned disks, ublk-based vs native), and hands-on (testing and
inspecting BlueStore in isolation, no cluster needed). The target reader
knows storage and kernel programming but is new to Ceph.
...
(Part 3); the two to highlight for learning, both offline:
...
exact code paths from Part 3 — set them via env on the standalone
tools:
