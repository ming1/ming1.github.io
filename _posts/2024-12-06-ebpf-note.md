...
```
YES, BPF maps are GLOBAL (system-wide), not per-task or per-process.

How BPF Maps Work

- Single instance per map: Each BPF map is a single object in kernel memory
- Shared via file descriptors: Multiple processes can access the same map by sharing/passing the FD
- Reference counted: atomic64_t refcnt tracks how many references exist
- Persistent: Maps can outlive the processes that created them (if pinned to bpffs)
...
```
BPF Arena is GLOBAL (one instance per map), but can be mapped into multiple processes at the same address.

Arena Architecture

From kernel/bpf/arena.c:

┌──────────────────────────────────────────────────────────────┐
│ BPF Arena: Shared Memory Between Kernel and User Processes  │
└──────────────────────────────────────────────────────────────┘

           KERNEL SIDE                    USER SIDE

    ┌─────────────────────┐         ┌─────────────────┐
    │  Kernel VM Area     │         │  Process A VMA  │
    │  (4GB region)       │◄───────►│  (mmap'd region)│
    │  ffffc90001e69000   │  shared │  0x7f7d26200000 │
    └─────────────────────┘  pages  └─────────────────┘
             ▲
             │                       ┌─────────────────┐
             │                       │  Process B VMA  │
             └──────────────────────►│  (mmap'd region)│
                       shared pages  │  0x7f7d26200000 │ (same addr!)
                                     └─────────────────┘

    All processes share the SAME kernel-backed pages

| Aspect               | Behavior                                       |
|----------------------|------------------------------------------------|
| Arena instance       | Global (one per map)                           |
| Kernel backing pages | Global (shared by all processes)               |
| User mappings        | Per-process VMA, but at same address           |
| Data visibility      | All processes see the same data                |
| Page faults          | Handled globally, pages inserted into all VMAs |
```
...
```
  Complete struct_ops Lifecycle

  ┌─────────────────────────────────────────────────────────────────────┐
  │ Timeline: When is .reg() Called?                                   │
  └─────────────────────────────────────────────────────────────────────┘

  Time  Userspace API                  Kernel Action                .reg()
  ────  ──────────────────────────     ─────────────────────────    ──────

  T0    skel = obj__open_and_load()    Load BPF programs            
        │                               (trampolines created)         
        │                                                             
        │                                                             
  T1    link = bpf_map__attach_        BPF_MAP_UPDATE_ELEM          
        │         struct_ops(map)       (if !BPF_F_LINK)             ✅ CALLED
        │                               OR                           (kdata, NULL)
        │                               BPF_LINK_CREATE              ✅ CALLED
        │                               (if BPF_F_LINK)              (kdata, &link)
        │                                                             
        │                               ┌─ struct_ops registered    
        │                               │  with subsystem            
        │                               │  (e.g., TCP stack)         
        │                                                             
  T2    bpf_link__pin(link,            Pin link to bpffs            ❌ NOT called
        │    "/sys/fs/bpf/my_link")    (just creates inode)         
        │                                                             
        │  ── Process A exits ──       Link stays alive             
        │                               (refcount > 0)               
        │                                                             
  T3    link = bpf_link__open(         BPF_OBJ_GET                  ❌ NOT called
        │    "/sys/fs/bpf/my_link")    - Lookup existing link       
        │                               - Increment refcount         
        │                               - Return new FD              
        │                                                             
        │                               Link still registered        
        │                               (same instance)              
        │                                                             
  T4    bpf_link__destroy(link)        Close FD                     ❌ NOT called
        │                               Decrement refcount           
        │                                                             
        │  ── If refcount > 0 ──       Link still alive             
        │  ── If refcount == 0 ──      Link cleanup triggered       
        │                                                             
  T5    (last FD closed or unpinned)   link->ops->release()         ✅ .unreg()
                                         └─> st_ops->unreg()         CALLED
                                                                     (kdata, link)
                                        Unregister from subsystem    

  ---
  Proof: Code Locations

  .reg() is called ONLY in two places:

  1. bpf_struct_ops_map_update_elem() at kernel/bpf/bpf_struct_ops.c:870
  if (!(st_map->map.map_flags & BPF_F_LINK)) {
      err = st_ops->reg(kdata, NULL);  // ← Legacy mode
  }
  2. bpf_struct_ops_link_create() at kernel/bpf/bpf_struct_ops.c:1379
  err = st_map->st_ops_desc->st_ops->reg(st_map->kvalue.data, &link->link);  // ← Link mode

  bpf_link__open() calls:
  - bpf_obj_get() → bpf_obj_get_user() → bpf_obj_do_get() → bpf_any_get()
  - Which for links just calls bpf_link_inc() → atomic64_inc(&link->refcnt)
  - No .reg() anywhere in this path!
```
...
```
  The prog_fd is the file descriptor returned from loading the BPF program. Here's the flow:

  1. Program loading returns an FD:
  fd = bpf_prog_load(...);  // Returns program FD
  2. High-level attachment functions get the FD from the program object:
  prog_fd = bpf_program__fd(prog);
  3. Pass the FD to link creation:
...
### how to share single struct_ops for multiple applications
...
[Unprivileged bpf()](https://lwn.net/Articles/660331/)

```
According to BPF developer Alexei Starovoitov, there is just one thing that is 
missing: ensuring that a BPF program cannot leak information about the kernel — 
and kernel pointers in particular — to user space. This information can be highly 
useful to an attacker trying to exploit a vulnerability, so quite a bit of effort 
has gone into plugging such leaks in recent years. BPF programs do not have access 
to a great deal of kernel information, but a hostile program still might succeed 
in exfiltrating something that an attacker can use. That makes it dangerous to 
allow unprivileged users to load and run BPF programs.
